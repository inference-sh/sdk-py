...
class HTTPToolBuilder(_ToolBuilder):
    ...
    def auth(self, *, credential: Optional[str] = None, credential_id: Optional[str] = None,
        ...
        if provider:
            auth = cast(ToolAuthConfig, {"type": ToolAuthType.CREDENTIAL, "provider": provider})
...
class LifecycleHookBuilder:
    ...
    def build(self) -> LifecycleHookConfig:
        ...
        if self._async is not None:
            config["async"] = self._async  # type: ignore[typeddict-unknown-key]  # "async" is a Python keyword; TypedDict uses async_ but wire format is "async"
